...
from .dataset_util import (
    ...
    resize_image_depth_layout_depth_and_intrinsic,
    resize_image_depth_layout_depth_seg_and_intrinsic,
    resize_image_depth_layout_depth_seg_normals_and_intrinsic,
    rotate_90_degrees,
    rotate_90_degrees_w_layout_depth,
    rotate_90_degrees_w_layout_depth_and_seg,
    rotate_90_degrees_w_layout_depth_seg_and_normals,
    depth_to_world_coords_points,
    threshold_depth_map,
    adjust_intrinsic_matrix_rot90,
    adjust_extrinsic_matrix_rot90,
    adjust_track_rot90,
    rotate_image_and_depth_rot90,
)
...
class BaseDataset(Dataset):
    ...
    def process_one_image_w_layout_depth(
        self,
        image,
        depth_map,
        layout_depth_map,
        ...
        """
        Identical pipeline to process_one_image, applied to both depth_map and
        layout_depth_map so they stay perfectly aligned with the RGB image.
        """
        # Make copies to avoid in-place operations affecting original data
        image = np.copy(image)
        depth_map = np.copy(depth_map)
        layout_depth_map = np.copy(layout_depth_map)
        extri_opencv = np.copy(extri_opencv)
        intri_opencv = np.copy(intri_opencv)
        if track is not None:
            track = np.copy(track)
        ...
        # Move principal point to the image center and crop if necessary.
        # crop_image_depth_and_intrinsic_by_pp is deterministic and does not mutate
        # its inputs (it copies intrinsic internally), so plain references here are safe.
        pre_crop_image = np.copy(image)
        pre_crop_intri = np.copy(intri_opencv)
        image, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, aug_size, track=track, filepath=filepath,
        )
        _, layout_depth_map, _, _ = crop_image_depth_and_intrinsic_by_pp(
            pre_crop_image, layout_depth_map, pre_crop_intri, aug_size,
        )
        ...
        # Resize images and update intrinsics
        if self.rescale:
            image, depth_map, layout_depth_map, intri_opencv, track = (
                resize_image_depth_layout_depth_and_intrinsic(
                    image,
                    depth_map,
                    layout_depth_map,
        ...
        _, layout_depth_map, _, _ = crop_image_depth_and_intrinsic_by_pp(
            pre_crop_image, layout_depth_map, pre_crop_intri, target_shape, strict=True,
        )
        ...
        layout_depth_mask = layout_depth_map > 1e-6
    ...
    def process_one_image_w_layout_depth_and_seg(
        self,
        image,
        depth_map,
        layout_depth_map,
        ...
        """
        Identical pipeline to process_one_image_w_layout_depth, extended to also
        carry a seg_mask (integer class IDs, uint8) through the same crop/resize/
        rotate steps so all outputs stay perfectly pixel-aligned.

        seg_mask is always resized with nearest-neighbor to preserve class IDs.
        """
        image = np.copy(image)
        depth_map = np.copy(depth_map)
        layout_depth_map = np.copy(layout_depth_map)
        ...
        # Random scale augmentation (training only)
        if self.training and self.aug_scale:
            random_h_scale, random_w_scale = np.random.uniform(
                self.aug_scale[0], self.aug_scale[1], 2
            )
        ...
        # --- Crop (deterministic given aug_size) ---
        pre_crop_image = np.copy(image)
        pre_crop_intri = np.copy(intri_opencv)
        image, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, aug_size, track=track, filepath=filepath,
        ...
        _, seg_mask, _, _ = crop_image_depth_and_intrinsic_by_pp(
            pre_crop_image, seg_mask, pre_crop_intri, aug_size,
        )
        ...
        # --- Resize (single random boundary shared across all maps) ---
        if self.rescale:
            image, depth_map, layout_depth_map, seg_mask, intri_opencv, track = (
                resize_image_depth_layout_depth_seg_and_intrinsic(
                    image,
                    depth_map,
                    layout_depth_map,
        ...
        # --- Final crop to exact target shape ---
        pre_crop_image = np.copy(image)
        pre_crop_intri = np.copy(intri_opencv)
        image, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, target_shape, track=track,
        ...
        _, seg_mask, _, _ = crop_image_depth_and_intrinsic_by_pp(
            pre_crop_image, seg_mask, pre_crop_intri, target_shape, strict=True,
        )
    ...
    def process_one_image_w_layout_depth_seg_and_normals(
        self,
        image,
        depth_map,
        layout_depth_map,
        ...
        """Extends process_one_image_w_layout_depth_and_seg to carry layout_normal_map
        through the identical crop / resize / rotate pipeline.

        layout_normal_map is resized with bilinear interpolation and renormalized after
        resize.  If 90-degree rotation is applied, vector channels are corrected so the
        normals remain consistent with the new image orientation (OpenCV convention:
        x=right, y=down, z=forward).

        Returns all outputs of process_one_image_w_layout_depth_and_seg plus:
          layout_normal_map: (H, W, 3) float32, unit-length where valid
          layout_normal_mask, (H, W) bool: layout_depth valid AND normal non-zero
        """
        image = np.copy(image)
        depth_map = np.copy(depth_map)
        layout_depth_map = np.copy(layout_depth_map)
        seg_mask = np.copy(seg_mask)
        layout_normal_map = np.copy(layout_normal_map)
        extri_opencv = np.copy(extri_opencv)
        intri_opencv = np.copy(intri_opencv)
        if track is not None:
            track = np.copy(track)
        ...
        # --- First crop (principal-point, non-strict) ---
        pre_crop_image = np.copy(image)
        pre_crop_intri = np.copy(intri_opencv)
        image, depth_map, intri_opencv, track = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, aug_size, track=track, filepath=filepath,
        ...
        # Normals are (H, W, 3), pass as the image argument so the 3-channel
        # crop path is used; depth_map=None so the 2-D branch is skipped.
        layout_normal_map, _, _, _ = crop_image_depth_and_intrinsic_by_pp(
            layout_normal_map, None, pre_crop_intri, aug_size,
        )
        ...
        layout_normal_map, _, _, _ = crop_image_depth_and_intrinsic_by_pp(
            layout_normal_map, None, pre_crop_intri, target_shape, strict=True,
        )
        ...
        layout_depth_mask = layout_depth_map > 1e-6
        layout_normal_mask = (
            (layout_depth_map > 1e-6)
            & (np.linalg.norm(layout_normal_map, axis=-1) > 1e-6)
        )
