...
try:
    # track_util is only required when load_track=True (e.g. CO3D track
    # supervision). Room Envelopes configs run with load_track=False and don't
    # ship the module; keep the module import optional so the file loads.
    from .track_util import *  # noqa: F401,F403
...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        # Optional layout-depth / room-envelope targets
        layout_depths = None
        layout_depth_masks = None
        layout_masks = None

        if "layout_depths" in batch and batch["layout_depths"] is not None:
            layout_depths = torch.from_numpy(
                np.stack(batch["layout_depths"]).astype(np.float32)
            )

        if "layout_depth_masks" in batch and batch["layout_depth_masks"] is not None:
            layout_depth_masks = torch.from_numpy(
                np.stack(batch["layout_depth_masks"])
            )

        # Binary layout mask target:
        # 1 = visible structural envelope, 0 = occluded / invalid
        if "layout_masks" in batch and batch["layout_masks"] is not None:
            layout_masks = torch.from_numpy(
                np.stack(batch["layout_masks"]).astype(np.float32)
            )

        # Layout surface normals: (S, H, W, 3) float32
        layout_normals = None
        if "layout_normals" in batch and batch["layout_normals"] is not None:
            layout_normals = torch.from_numpy(
                np.stack(batch["layout_normals"]).astype(np.float32)
            )

        # Layout normal validity mask: (S, H, W) bool
        layout_normal_masks = None
        if "layout_normal_masks" in batch and batch["layout_normal_masks"] is not None:
            layout_normal_masks = torch.from_numpy(
                np.stack(batch["layout_normal_masks"])
            )

        # Optional segmentation targets (structural 4-class masks, int64 for cross-entropy)
        seg_masks = None
        if "seg_masks" in batch and batch["seg_masks"] is not None:
            seg_masks = torch.from_numpy(
                np.stack(batch["seg_masks"]).astype(np.int64)
            )
        ...
        if layout_depths is not None:
            sample["layout_depths"] = layout_depths

        if layout_depth_masks is not None:
            sample["layout_depth_masks"] = layout_depth_masks

        if layout_masks is not None:
            sample["layout_masks"] = layout_masks

        if seg_masks is not None:
            sample["seg_masks"] = seg_masks

        if layout_normals is not None:
            sample["layout_normals"] = layout_normals

        if layout_normal_masks is not None:
            sample["layout_normal_masks"] = layout_normal_masks
