...
def resize_image_depth_layout_depth_and_intrinsic(
    image,
    depth_map,
    layout_depth_map,
    intrinsic,
    ...
    if layout_depth_map is not None:
        layout_depth_map = cv2.resize(
            layout_depth_map,
            tuple(output_resolution),
            interpolation=cv2.INTER_NEAREST,
        )
    ...
    if layout_depth_map is not None:
        assert image.shape[:2] == layout_depth_map.shape[:2]

    return image, depth_map, layout_depth_map, intrinsic, track

def resize_image_depth_layout_depth_seg_and_intrinsic(
    image,
    depth_map,
    layout_depth_map,
    seg_mask,
    ...
    """
    Identical to resize_image_depth_layout_depth_and_intrinsic but also resizes
    seg_mask with nearest-neighbor to preserve integer class labels.
    """
    if rescale_aug:
        random_boundary = np.random.triangular(0, 0, 0.3)
        safe_bound = safe_bound + random_boundary * target_shape.max()
    ...
    if layout_depth_map is not None:
        assert image.shape[:2] == layout_depth_map.shape[:2]
    if seg_mask is not None:
        assert image.shape[:2] == seg_mask.shape[:2]

    return image, depth_map, layout_depth_map, seg_mask, intrinsic, track


def resize_image_depth_layout_depth_seg_normals_and_intrinsic(
    image,
    depth_map,
    layout_depth_map,
    seg_mask,
    ...
    """Extends resize_image_depth_layout_depth_seg_and_intrinsic to also resize
    layout_normal_map with bilinear interpolation and renormalize vectors.
    The random safe_bound offset is shared so all maps stay pixel-aligned.
    """
    if rescale_aug:
        random_boundary = np.random.triangular(0, 0, 0.3)
        safe_bound = safe_bound + random_boundary * target_shape.max()
    ...
    if depth_map is not None:
        depth_map = cv2.resize(
            depth_map, tuple(output_resolution), interpolation=cv2.INTER_NEAREST,
        )

    if layout_depth_map is not None:
        layout_depth_map = cv2.resize(
            layout_depth_map, tuple(output_resolution), interpolation=cv2.INTER_NEAREST,
        )

    if seg_mask is not None:
        seg_mask = cv2.resize(
            seg_mask, tuple(output_resolution), interpolation=cv2.INTER_NEAREST,
        )

    if layout_normal_map is not None:
        layout_normal_map = cv2.resize(
            layout_normal_map, tuple(output_resolution), interpolation=cv2.INTER_LINEAR,
        )
        norms = np.linalg.norm(layout_normal_map, axis=-1, keepdims=True).clip(1e-8)
        layout_normal_map = (layout_normal_map / norms).astype(np.float32)
    ...
    intrinsic[:2, :] *= actual_resize_scale
    ...
    if seg_mask is not None:
        assert image.shape[:2] == seg_mask.shape[:2]
    if layout_normal_map is not None:
        assert image.shape[:2] == layout_normal_map.shape[:2]

    return image, depth_map, layout_depth_map, seg_mask, layout_normal_map, intrinsic, track


def rotate_90_degrees_w_layout_depth_and_seg(
    image,
    depth_map,
    layout_depth_map,
    seg_mask,
    ...
    """Extends rotate_90_degrees_w_layout_depth to also rotate a seg_mask."""
    image_height, image_width = image.shape[:2]

    rotated_image, rotated_depth_map = rotate_image_and_depth_rot90(
        image, depth_map, clockwise
    )
    _, rotated_layout_depth_map = rotate_image_and_depth_rot90(
        image, layout_depth_map, clockwise
    )
    _, rotated_seg_mask = rotate_image_and_depth_rot90(
        image, seg_mask, clockwise
    )
...
def rotate_90_degrees_w_layout_depth_seg_and_normals(
    image,
    depth_map,
    layout_depth_map,
    seg_mask,
    ...
    """Extends rotate_90_degrees_w_layout_depth_and_seg to also rotate layout_normal_map.

    Spatial rotation uses rotate_image_and_depth_rot90 (normals passed as image argument
    since they are (H, W, 3)).  Vector channels are corrected for OpenCV camera convention
    (x=right, y=down, z=forward):
      Clockwise 90°:         new_nx = -old_ny, new_ny = old_nx, new_nz = old_nz
      Counter-clockwise 90°: new_nx =  old_ny, new_ny = -old_nx, new_nz = old_nz
    """
    image_height, image_width = image.shape[:2]
    ...
    if layout_normal_map is not None:
        # Pass normals as "image" so the (H, W, 3) transpose path is used; depth=None
        rotated_normal_map, _ = rotate_image_and_depth_rot90(
            layout_normal_map, None, clockwise
        )
        nx = rotated_normal_map[..., 0].copy()
        ny = rotated_normal_map[..., 1].copy()
        nz = rotated_normal_map[..., 2].copy()
        if clockwise:
            rotated_normal_map = np.stack([-ny, nx, nz], axis=-1).astype(np.float32)
        else:
            rotated_normal_map = np.stack([ny, -nx, nz], axis=-1).astype(np.float32)
    else:
        rotated_normal_map = None
...
def rotate_90_degrees_w_layout_depth(
    image,
    depth_map,
    layout_depth_map,
    extri_opencv,
