...
def normalize_camera_extrinsics_and_points_batch(
    ...
    layout_depths: Optional[torch.Tensor] = None,
    ...
    Args:
        ...
        layout_depths: Optional layout-depth maps of shape (B, S, H, W); normalised
            with the same scale factor as depths.
    ...
    Returns:
        ...
        - Normalized layout_depths (same shape as input layout_depths, or None)
    """
    # Validate inputs
    ...
    if scale_by_points:
        ...
        # Apply the same scale to layout_depths (same coordinate frame as depths)
        new_layout_depths = None
        if layout_depths is not None:
            new_layout_depths = layout_depths.clone() / avg_scale.view(-1, 1, 1, 1)
            new_layout_depths = check_and_fix_inf_nan(new_layout_depths, "new_layout_depths", hard_max=None)
    else:
        return new_extrinsics[:, :, :3], cam_points, new_world_points, depths, layout_depths
    ...
    return new_extrinsics, new_cam_points, new_world_points, new_depths, new_layout_depths
