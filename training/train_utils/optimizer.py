...
def log_optimizer_param_groups(optimizer_wrappers: Union[List[OptimizerWrapper], "OptimizerWrapper", None],
                                model: nn.Module,
                                log_file: str = None) -> None:
    """Log each param group's parameter count, # trainable, and current LR / weight_decay.

    Helpful for verifying that, e.g., the unfrozen aggregator blocks landed in
    their own group with the intended lower learning rate.
    """
    if optimizer_wrappers is None:
        return
    if not isinstance(optimizer_wrappers, list):
        optimizer_wrappers = [optimizer_wrappers]

    # Reverse map: tensor id -> param name (best-effort match).
    name_by_id = {id(p): n for n, p in model.named_parameters()}

    sep = "=" * 78
    lines = [sep, "Optimizer parameter groups", sep]
    for opt_idx, wrapper in enumerate(optimizer_wrappers):
        opt = wrapper.optimizer if hasattr(wrapper, "optimizer") else wrapper
        lines.append(f"  Optimizer[{opt_idx}]: {type(opt).__name__}")
        for gi, pg in enumerate(opt.param_groups):
            ...
            # Sample a few parameter names to identify what's in this group.
            sample_names = []
            for p in params[:3]:
            ...
            wd_str = f"{wd:.3e}" if isinstance(wd, float) else str(wd)
            lines.append(
                f"    group[{gi}]: lr={lr_str}  wd={wd_str}  "
                f"params={n_params:>12,}  trainable={n_trainable:>12,}  "
                f"(n_tensors={len(params)})"
            )
            if sample_names:
                lines.append(f"      sample params: {sample_names}")
    lines.append(sep)
    block = "\n".join(lines)
    print("\n" + block + "\n")
    ...
    if log_file:
        try:
            ...
            logging.warning(
                "Could not write optimizer-param-groups summary to %s: %s",
                log_file, exc,
            )
